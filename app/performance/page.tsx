...
export default function PerformancePage() {
    ...
    const getMetricStatus = (metric: string, value: number) => {
        switch (metric) {
            case 'ttfr':
                if (value <= 2) return { color: 'text-green-400', status: '🎯 META ATINGIDA!' };
                if (value <= 5) return { color: 'text-yellow-400', status: '⚠️ Próximo da meta' };
                return { color: 'text-red-400', status: '❌ Acima da meta' };
            case 'ttr':
                if (value <= 2) return { color: 'text-green-400', status: '🎯 META ATINGIDA!' };
                if (value <= 4) return { color: 'text-yellow-400', status: '⚠️ Próximo da meta' };
                return { color: 'text-red-400', status: '❌ Acima da meta' };
            case 'automation':
                if (value >= 95) return { color: 'text-green-400', status: '🚀 MUSK LEVEL!' };
                if (value >= 70) return { color: 'text-yellow-400', status: '⚡ Bom desempenho' };
                return { color: 'text-red-400', status: '❌ Abaixo da meta' };
            case 'accuracy':
                if (value >= 90) return { color: 'text-green-400', status: '🎯 META ATINGIDA!' };
                if (value >= 70) return { color: 'text-yellow-400', status: '⚠️ Próximo da meta' };
                return { color: 'text-red-400', status: '❌ Abaixo da meta' };
            default:
                return { color: 'text-gray-400', status: '' };
        }
    };
                        ...
                        <>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                                ...
                                <div className="bg-gray-900 p-6 rounded-lg border border-gray-800">
                                    ...
                                    <p className="text-sm text-gray-400">TTFR (Time to First Response)</p>
                                ...
                                <div className="bg-gray-900 p-6 rounded-lg border border-gray-800">
                                    ...
                                    <p className="text-sm text-gray-400">TTR (Time to Resolution)</p>
                                ...
                                <div className="bg-gray-900 p-6 rounded-lg border border-gray-800">
                                    ...
                                    <p className="text-sm text-gray-400">Taxa de Automação</p>
                                ...
                                <div className="bg-gray-900 p-6 rounded-lg border border-gray-800">
                                    ...
                                    <p className="text-sm text-gray-400">Precisão da IA</p>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                                <div className="bg-gray-900 p-6 rounded-lg border border-gray-800">
                                    <p className="text-sm text-gray-400">Total Issues (24h)</p>
                                    <p className="text-3xl font-bold text-white mt-1">{metrics?.total_issues_24h}</p>
                                </div>

                                <div className="bg-gray-900 p-6 rounded-lg border border-gray-800">
                                    <p className="text-sm text-gray-400">Resolvidos (24h)</p>
                                    <p className="text-3xl font-bold text-green-400 mt-1">{metrics?.resolved_issues_24h}</p>
                                </div>

                                <div className="bg-gray-900 p-6 rounded-lg border border-gray-800">
                                    <p className="text-sm text-gray-400">Auto-Resolvidos (24h)</p>
                                    <p className="text-3xl font-bold text-blue-400 mt-1">{metrics?.auto_resolved_24h}</p>
                                </div>

                                <div className="bg-gray-900 p-6 rounded-lg border border-gray-800">
                                    <p className="text-sm text-gray-400">Críticos Pendentes</p>
                                    <p className={`text-3xl font-bold mt-1 ${(metrics?.critical_issues_pending || 0) > 0 ? 'text-red-400' : 'text-green-400'}`}>
                            ...
                            <div className="bg-gray-900 p-6 rounded-lg border border-gray-800 mb-8">
                                <h2 className="text-xl font-bold text-white mb-4">📊 Últimas 12 Horas</h2>
                                        ...
                                        return (
                                            <div key={idx} className="space-y-1">
                                                <div className="flex items-center gap-4">
                                                    <span className="text-sm font-mono text-gray-500 w-12">{data.hour}</span>
                            ...
                            {/* Performance Status */}
                            <div className="bg-gradient-to-r from-indigo-600/20 to-purple-600/20 p-6 rounded-lg border border-indigo-500/30 text-white">
                                <h2 className="text-xl font-bold mb-4">🎯 Status Geral</h2>
                                <div className="space-y-2">
                                    {metrics && (
                                        <>
                                            {metrics.ttfr_avg <= 2 && metrics.automation_rate >= 95 && (
                                                <p className="text-lg font-semibold text-green-400">🚀 MUSK LEVEL ATINGIDO! Sistema operando em performance máxima!</p>
                                            )}
                                            {metrics.ttfr_avg <= 2 && metrics.automation_rate < 95 && (
                                                <p className="text-lg font-semibold text-yellow-400">⚡ Excelente TTFR! Foco agora em aumentar automação para 95%.</p>
                                            )}
                                            {metrics.ttfr_avg > 2 && metrics.automation_rate >= 95 && (
                                                <p className="text-lg font-semibold text-yellow-400">🎯 Alta automação! Otimizar TTFR para &lt;2min.</p>
                                            )}
                                            {metrics.ttfr_avg > 2 && metrics.automation_rate < 95 && (
                                                <p className="text-lg font-semibold text-orange-400">📈 Oportunidades de melhoria em TTFR e automação.</p>
                                            )}
                                        </>
                                    )}
                                </div>
